const sqlConfig = require("./dbConfig.js"); // DATA NA SPOJENIE S DATABAZOU
        ...
        if (result.recordsets[0].length > 0) {
            for (let i = 0; i < result.recordsets[0].length; i++) {
                ...
                dataArray.push({
                    ...
                    operatorName: operatorName[i],
                    quantity: quantity[i],
                    idMixer: idMixer[i],
                    mixingTime: mixingTime[i],
                    idPackingMachine: idPackingMachine[i],
...
async function getStatDose() {
    try {
        ...
        let no = [], datetime = [], name = [], componentSP = [], componentPV = [], dataArray = [], idContainer = [],
            idOrder = [];
        if (result.recordsets[0].length > 0) {
            for (let i = 0; i < result.recordsets[0].length; i++) {
                no[i] = result.recordsets[0][i].no; //
                ...
                dataArray.push({
                    ...
                    datetime: datetime[i],
                    name: name[i],
                    componentSP: componentSP[i],
                    componentPV: componentPV[i],
                    idContainer: idContainer[i],
                    idOrder: idOrder[i]
                })
